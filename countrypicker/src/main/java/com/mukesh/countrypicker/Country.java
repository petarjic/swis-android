...
public class  Country {
  ...
  public static List<Country> getAllCountriesSortedByCode() {
      List<Country> allCountriesList = Arrays.asList(COUNTRIES);
      Collections.sort(allCountriesList,new Comparator<Country>() {
        @Override
        public int compare(Country country, Country t1) {
  ...
  public static Country getCountryByISO(String countryIsoCode) {
    ...
    int i = Collections.binarySearch(getAllCountriesSortedByCode(), c, new ISOCodeComparator());


    if (i < 0) {
      return getAllCountriesSortedByCode().get(0);
    } else {
      return getAllCountriesSortedByCode().get(i);
    }
  }
